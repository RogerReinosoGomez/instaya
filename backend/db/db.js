...
const conectarMongoDb = async () => {
    try {
        console.log('Conectado con DB InstaYa mediante Mongoose...');
        await mongoose.connect(process.env.MONGODB_URI);
        console.log('Conexion exitosa.');
    } catch (error) {
        console.log('Error al conectar a la DB InstaYa.');
        console.log(error);
    }
}
...
const desconectarMongoDb = async () => {
    if (mongoose.connection) {
        try {
            console.log('Cerrando conexion de DB InstaYa mediante Mongoose...');
            await mongoose.connection.close();
            console.log('Desconexion exitosa.');
        } catch (error) {
            console.log('Error al desconectar a la DB InstaYa.');
            console.log(error);
        }
    }
}
